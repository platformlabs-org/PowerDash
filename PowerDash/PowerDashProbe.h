...
// 测试中用 FixtureDriverIo 注入应答。SMN 访问(k10temp Tctl 直读与
// SMU 邮箱)统一走 ReadSmn/WriteSmn(驱动内原子互斥的
// IO_CTL_SMN_READ/WRITE,Hal 0x60/0x64 窗,读与写均实证可用);
// 用户态 ECAM 绕道已删(\Device\PhysicalMemory 为 WB 缓存映射,访问
// MMIO 未定义且三次 bugcheck,详见 PowerDashPmTable.h 留档)。
class DriverIo {
public:
