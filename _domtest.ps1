...
foreach ($f in 'idle','load') {
  "=== $f ==="
  $txt = & $strip ([IO.File]::ReadAllText("C:\Users\labs\$f.bin"))
  ($txt -split "`n") | Where-Object { $_ -match 'PACKAGE POWER|of PL2|= PL1|IA |GT |REST|SYSTEM|UTIL' } | ForEach-Object { $_.TrimEnd() } | Select-Object -First 10
}
